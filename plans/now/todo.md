...
All five landed 2026-08-30 (ad31c17..0438a03). Scope was **safe wins only** — five items, no missing hardware, each landing on
its own. In order:

- [x] **Friendly app display names.** New `wavexlr/wmnames.py` (X11
...
- [x] **Shared `Throttler`.** New `wavexlr/scheduler.py`; replace the three
...
- [x] **Duplicate-source picker guard.** `app.py` has `_bound_capture_nodes()`
...
- [x] **ALSA numid discovery by control-name suffix.** `device.py` hardcodes
...
- [x] **Device hotplug auto-reconnect.** Only a manual Refresh button today
