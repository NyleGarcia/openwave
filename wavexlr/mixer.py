...
def fx_node_name(source_id):
    """The virtual Source a microphone's DSP chain publishes.

    When any effect is on, the row's cells capture this node instead of
    the raw device — the same virtual-microphone shape PipeWire's own
    filter-chain documentation uses.
    """
    return f"{FX_NODE_PREFIX}{source_id}"


def fx_config_path(source_id):
    """Where a source's generated filter-chain config lives.

    Beside the cell store on purpose: the tests redirect CONFIG_PATH into
    a sandbox, and the configs must follow it there.
    """
    return os.path.join(os.path.dirname(CONFIG_PATH), "fx",
                        f"{source_id}.conf")


def render_fx_config(source):
    """A pipewire.conf that hosts one filter-chain for this source's fx.

    The chain is built only from what is non-neutral, in fixed order:
    high-pass, then the three tone bands, then delay. A mono-only chain
    is a single copy node — the downmix is the streams being one channel,
    not a plugin. Spawned as `pipewire -c <this>`: a module must live in
    some process, and a config-owned child fits the same lifecycle rules
    as every pw-loopback here.
    """
    f = sources.fx(source)
    nodes, controls = [], []
    if f["lowcut"]:
    ...
    if f["eq_mid"]:
        nodes.append(("eqm", "bq_peaking",
                      f'control = {{ "Freq" = 1000.0 "Gain" = {float(f["eq_mid"]):.1f} }}'))
    if f["eq_high"]:
        nodes.append(("eqh", "bq_highshelf",
                      f'control = {{ "Freq" = 8000.0 "Gain" = {float(f["eq_high"]):.1f} }}'))
    if f["delay_ms"]:
        secs = max(0.0, min(1.0, float(f["delay_ms"]) / 1000.0))
        nodes.append(("dly", "delay",
                      'config = { "max-delay" = 1.0 } '
                      f'control = {{ "Delay (s)" = {secs:.4f} }}'))
    if not nodes:
    ...
    node = fx_node_name(source["id"])
    raw = source.get("node_name", "")
    # One channel end to end: every supported microphone is mono, and for
    # a stereo capture this IS the mono downmix toggle's mechanism.
    # The module preamble mirrors the stock filter-chain.conf: a bare
    # `pipewire -c` context has no protocol-native and cannot even connect
    # to the daemon without it.
    return f"""# Generated by OpenWave — one DSP chain for "{label}". Do not edit.
...
context.modules = [
    ...
    {{ name = libpipewire-module-filter-chain
        args = {{
            node.description = "OpenWave FX: {label}"
            media.name = "OpenWave FX: {label}"
            filter.graph = {{
                nodes = [
{node_lines}
                ]
{"                links = [" + chr(10) + link_lines + chr(10) + "                ]" if link_lines else ""}
            }}
            audio.channels = 1
            audio.position = [ MONO ]
            capture.props = {{
                ...
                node.description = "OpenWave FX: {label} (capture)"
            }}
            playback.props = {{
                node.name = "{node}"
...
class Mixer:
    ...
    def _drop_device_cell_loopbacks(self, nodes):
        ...
        for sid in sids:
            ...
            # The fx chain captured the corpse too; forget its config hash
    ...
    def _reconcile_all(self):
        ...
        with self._lock:
            ...
            mix_ids = list(self._mixes)
        for source_id in source_ids:
            self._reconcile_fx(source_id)
        for source_id in source_ids:
            for mix_id in mix_ids:
                self._reconcile_cell(source_id, mix_id)

    def _fx_key(self, source_id):
        # Shaped (source_id, ...) so _do_remove_source's prefix sweep and
        # the replug teardown catch it with the cell loopbacks.
        return (source_id, "__fx__")

    def _reconcile_fx(self, source_id):
        """Keep one filter-chain process matching the source's fx settings.

        Neutral settings hold no process. A settings change respawns —
        the chain rebuilds in well under a second, and a respawn is the
        one code path, where live parameter patching would be a second
        one that drifts.
        """
        source = self._sources.get(source_id)
        key = self._fx_key(source_id)
        wanted = (source is not None
                  and sources.kind(source) == sources.KIND_DEVICE
                  and sources.fx_active(source)
                  and bool(source.get("node_name")))
        if not wanted:
            self._destroy_loopback(key)
        ...
        with open(path, "w") as handle:
            handle.write(conf)
        spawned = self._pw.spawn_loopback(["pipewire", "-c", path], False)
        if spawned is None:
            return
        self._procs[key] = spawned
        self._fx_conf[source_id] = conf
    ...
    def _reconcile_cell(self, source_id, mix_id):
        ...
        if source is not None and sources.kind(source) == sources.KIND_DEVICE:
            capture_node = source.get("node_name")
            fx_proc = self._procs.get(self._fx_key(source_id))
            if sources.fx_active(source) \
                    and fx_proc is not None and fx_proc.poll() is None:
                # The cells drink from the DSP chain's published Source,
                # not the raw device — that is the whole insertion.
                capture_node = fx_node_name(source_id)
            self._reconcile_capture_cell(
                source_id, mix_id, capture_node,
    ...
    def _reconcile_capture_cell(self, source_id, mix_id, capture_node, volume, muted):
        ...
        mix_sink = self._mix_sink(mix_id)
        live = self._live_captures
        # An FX chain's node is ours: its process is the presence check,
        # and a freshly spawned chain must not read as "device absent" for
        # the seconds until the next pw-dump snapshot sees it.
        absent = (bool(live) and capture_node not in live
                  and not str(capture_node or "").startswith(FX_NODE_PREFIX))
